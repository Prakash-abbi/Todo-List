...
function App() {
  ...
  const onSubmitForm = async (e)=>{
    ...
    try {
      ...
      setError("Failed to add todo. plaese try again.")
  ...
  const getTodos =async () =>{
    try {
      ...
      setError("Failed to fatch todos.please try again later.");
    }finally{
      setLoading(false)
    }
  };
     ...
     {loading ? (
      <div>
        ...
        <div className="flex flex-col gap-y-4">
          ...
          <div key={todo.todo_id} className="p-3">
             {editingTodo === todo.todo_id ? (
              <div className="flex items-center gap-x-3">
                <input className=" flex-1 p-3 border rounded-lg border-gray-200 outline focus:ring-2 focus:ring-blue-300 text-gray-700 shadow-inner" 
                type="text"
                value={editedText}
           ...
           <button onClick={()=>toggleCompleted(todo.todo_id)} className={`flex-shrink-0 h-6 w-6 border-2 rounded-full
